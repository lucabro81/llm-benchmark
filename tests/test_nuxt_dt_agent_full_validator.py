"""Tests for src/agent/nuxt_dt_agent_full/validator.py.
...
In the full variant, types + columns live in separate files — validate_ast_structure
receives combined code (vue + columns.ts + types.ts) so all checks still pass.
"""
...
from src.agent.nuxt_dt_agent_full.validator import (
    ASTResult,
    CompilationResult,
    NamingResult,
...
class TestValidateAstStructure:
    ...
    def test_combined_three_files_scores_ten(self):
        """Validator must work on combined code from all three files (types + columns + vue)."""
        types_code = "export type OrderStatus = 'pending' | 'delivered';\nexport interface Order { id: string; customer: string; status: OrderStatus; items: number; total: number; date: string; }"
        columns_code = "import { h } from 'vue';\nconst statusClasses: Record<OrderStatus, string> = {};\nexport const createColumns = (handlers: { onView: any; onCancel: any }) => [\n  { id: 'id' }, { id: 'customer' }, { id: 'status' }, { id: 'total' }, { id: 'date' }, { id: 'actions' },\n];"
        combined = types_code + "\n\n// --- columns.ts ---\n" + columns_code + "\n\n// --- OrdersDataTable.vue ---\n" + COMPLETE_COMPONENT
        result = validate_ast_structure(combined, SPEC)
        assert result.score == pytest.approx(10.0)
