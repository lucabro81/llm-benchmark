"""Tests for src/agent/veevalidate_zod_form/test_runner.py — AgentTest orchestrator.

TDD Red phase: all tests fail until test_runner.py is implemented.

Mocks used:
- src.agent.veevalidate_zod_form.test_runner.run_agent  → AgentRunResult-like SimpleNamespace
- src.agent.veevalidate_zod_form.test_runner.validator  → module mock with validate_* methods
"""
...
from src.agent.veevalidate_zod_form.test_runner import AgentBenchmarkResult, AgentTest
...
EMPTY_VUE = "<script setup lang='ts'>\n</script>\n\n<template>\n</template>\n"
...
DEFAULT_SPEC = {
    ...
    "allowed_write_paths": ["src/components/RegistrationForm.vue"],
    "max_steps": 20,
    "required_patterns": {
        "script_lang": "ts",
        "use_form": "useForm",
...
def _make_fixture(tmp_path, spec=None) -> Path:
    ...
    (fixture_path / "prompt.md").write_text("Implement a registration form component.")
    ...
    (comp_dir / "RegistrationForm.vue").write_text(EMPTY_VUE)
...
def _make_ast_result(score=10.0, missing=None, checks=None):
    return SimpleNamespace(
        score=score,
        missing=missing or [],
        checks=checks or {"script_lang": True, "use_form": True, "zod_schema": True},
    )
...
class TestAgentBenchmarkResult:
    ...
    def test_dict_is_json_serialisable(self):
        result = AgentBenchmarkResult(
            ...
            output_code="", errors=[], steps=2, max_steps=20, iterations=0,
...
class TestAgentTestInit:
    def test_loads_prompt_and_spec(self, tmp_path):
        ...
        assert test.validation_spec["target_file"] == "src/components/RegistrationForm.vue"
        assert test.fixture_name == "veevalidate-zod-form"

    def test_reads_max_steps_from_spec(self, tmp_path):
        fixture_path = _make_fixture(tmp_path)
        test = AgentTest(model="test-model", fixture_path=fixture_path)

        assert test.max_steps == 20

    def test_reads_allowed_paths_from_spec(self, tmp_path):
        fixture_path = _make_fixture(tmp_path)
        test = AgentTest(model="test-model", fixture_path=fixture_path)

        assert "src/components/RegistrationForm.vue" in test.allowed_paths
    ...
    def test_allowed_paths_defaults_to_target_file_if_missing_from_spec(self, tmp_path):
        ...
        assert test.allowed_paths == ["src/components/RegistrationForm.vue"]

    def test_stores_original_empty_code(self, tmp_path):
        fixture_path = _make_fixture(tmp_path)
        test = AgentTest(model="test-model", fixture_path=fixture_path)

        assert test.original_code == EMPTY_VUE
    ...
    def test_raises_on_missing_target_file(self, tmp_path):
        fixture_path = _make_fixture(tmp_path)
        (fixture_path / "target_project" / "src" / "components" / "RegistrationForm.vue").unlink()
...
class TestAgentTestRun:
    ...
    def test_result_contains_steps_and_succeeded(self, mock_run_agent, mock_validator, tmp_path):
        fixture_path = _make_fixture(tmp_path)
        mock_run_agent.return_value = _make_agent_result(steps=5, succeeded=True)
        mock_validator.validate_compilation.return_value = _make_compilation_result()
        mock_validator.validate_ast_structure.return_value = _make_ast_result()
        mock_validator.validate_naming.return_value = _make_naming_result()
    ...
    def test_iterations_counts_run_compilation_calls(self, mock_run_agent, mock_validator, tmp_path):
        ...
        log = [
            ...
            {"step": 2, "tool": "run_compilation", "args": {}, "result_summary": "Compilation succeeded."},
            {"step": 3, "tool": "write_file", "args": {}, "result_summary": "updated"},
            {"step": 4, "tool": "run_compilation", "args": {}, "result_summary": "Compilation succeeded."},
        ]
        mock_run_agent.return_value = _make_agent_result(steps=4, tool_call_log=log)
        mock_validator.validate_compilation.return_value = _make_compilation_result()
        mock_validator.validate_ast_structure.return_value = _make_ast_result()
        mock_validator.validate_naming.return_value = _make_naming_result()
    ...
    @patch("src.agent.veevalidate_zod_form.test_runner.validator")
    @patch("src.agent.veevalidate_zod_form.test_runner.run_agent")
    def test_zero_score_when_compilation_fails_and_no_patterns(self, mock_run_agent, mock_validator, tmp_path):
        fixture_path = _make_fixture(tmp_path)
        mock_run_agent.return_value = _make_agent_result()
        mock_validator.validate_compilation.return_value = _make_compilation_result(
            success=False, errors=["TS2307: Cannot find module 'vee-validate'"]
        )
    ...
    def test_original_file_restored_after_successful_run(self, mock_run_agent, mock_validator, tmp_path):
        """Target file must revert to original empty stub after run."""
        fixture_path = _make_fixture(tmp_path)
        target_file = fixture_path / "target_project" / "src" / "components" / "RegistrationForm.vue"

        def side_effect(*args, **kwargs):
            target_file.write_text(COMPLETE_VUE)
        ...
        assert target_file.read_text() == EMPTY_VUE
    ...
    def test_ast_exception_produces_degraded_result(self, mock_run_agent, mock_validator, tmp_path):
        ...
        mock_run_agent.return_value = _make_agent_result()
        mock_validator.validate_compilation.return_value = _make_compilation_result()
        mock_validator.validate_ast_structure.side_effect = Exception("regex parser crashed")
        mock_validator.validate_naming.return_value = _make_naming_result()
    ...
    def test_tool_call_log_in_result(self, mock_run_agent, mock_validator, tmp_path):
        fixture_path = _make_fixture(tmp_path)
        log = [{"step": 1, "tool": "write_file", "args": {}, "result_summary": "written"}]
    ...
    def test_passes_model_and_prompt_to_run_agent(self, mock_run_agent, mock_validator, tmp_path):
        ...
        assert model_arg == "qwen2.5-coder:7b"
        task_arg = (
            call_kwargs.kwargs.get("task")
            or (call_kwargs[0][1] if len(call_kwargs[0]) > 1 else None)
        )
        assert task_arg is not None
        assert "registration form" in task_arg.lower()
