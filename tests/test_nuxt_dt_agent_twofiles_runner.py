"""Tests for src/agent/nuxt_dt_agent_twofiles/test_runner.py.

Key difference from guided runner:
- output_code in result contains BOTH files: vue code + "// --- columns.ts ---" + columns code
- allowed_write_paths includes both OrdersDataTable.vue and columns.ts
- max_steps: 15
"""
...
from src.agent.nuxt_dt_agent_twofiles.test_runner import AgentBenchmarkResult, AgentTest
...
def _make_fixture(tmp_path: Path, spec=None) -> Path:
    ...
    fixture_path = tmp_path / "nuxt-dt-agent-twofiles"
    fixture_path.mkdir(exist_ok=True)
    (fixture_path / "prompt.md").write_text("Implement an orders data table with two files.")
...
def _make_agent_result(**kwargs):
    defaults = dict(
        ...
        tool_call_log=[
            {"step": 1, "tool": "write_file", "args": {}, "result_summary": "columns written"},
...
class TestAgentTestInit:
    ...
    def test_loads_prompt_and_spec(self, tmp_path):
        ...
        assert "data table" in test.prompt.lower()
...
class TestAgentTestTools:
    ...
    @patch("src.agent.nuxt_dt_agent_twofiles.test_runner.validator")
    @patch("src.agent.nuxt_dt_agent_twofiles.test_runner.run_agent")
...
class TestAgentTestRun:
    ...
    @patch("src.agent.nuxt_dt_agent_twofiles.test_runner.validator")
    @patch("src.agent.nuxt_dt_agent_twofiles.test_runner.run_agent")
    def test_combined_output_includes_columns_separator(self, mock_run_agent, mock_validator, tmp_path):
        """output_code must concatenate vue + columns with separator."""
        fixture_path = _make_fixture(tmp_path)
        ...
        result = AgentTest(model="m", fixture_path=fixture_path).run()
        assert "// --- columns.ts ---" in result.output_code
...
class TestWriteFileDecoupled:
    ...
    def test_write_file_returns_file_written_only(self, tmp_path):
        ...
        from src.agent.nuxt_dt_agent_twofiles.test_runner import _make_tools
