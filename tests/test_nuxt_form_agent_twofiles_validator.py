"""Tests for src/agent/nuxt_form_agent_twofiles/validator.py.

Same scoring rules as guided/nuxt-rag:
...
In the two-files variant, types live in a separate file — validate_ast_structure
receives combined code (vue + types) so all checks still pass.
"""
...
from src.agent.nuxt_form_agent_twofiles.validator import (
    ASTResult,
    CompilationResult,
    NamingResult,
...
import { registrationSchema, registrationInitialValues } from "@/registration/types";
...
<template>
  <Form :initial-values="registrationInitialValues" :form-schema="registrationSchema" :actions="actions">
...
export const registrationInitialValues: RegistrationFormValues = {
  username: "", email: "", role: "user", otherInfo: "", newsletter: false,
  frequency: undefined, bio: "",
};
"""

COMBINED_CODE = VUE_CODE + "\n// --- types/index.ts ---\n" + TYPES_CODE
...
class TestValidateAstStructure:
    ...
    def test_combined_code_scores_ten(self):
        assert validate_ast_structure(COMBINED_CODE, SPEC).score == pytest.approx(10.0)

    def test_returns_ast_result(self):
        assert isinstance(validate_ast_structure(COMBINED_CODE, SPEC), ASTResult)

    def test_empty_string_scores_zero(self):
        assert validate_ast_structure("", SPEC).score == pytest.approx(0.0)

    def test_zod_in_types_file_counts(self):
        """z.object in the types portion of combined code should be found."""
        assert "zod_schema" not in validate_ast_structure(COMBINED_CODE, SPEC).missing

    def test_missing_script_lang_reduces_score(self):
        code = COMBINED_CODE.replace('lang="ts"', 'lang="js"')
        assert "script_lang" in validate_ast_structure(code, SPEC).missing

    def test_missing_form_component_reduces_score(self):
        result = validate_ast_structure(COMBINED_CODE.replace("<Form ", "<FormWrapper "), SPEC)
        assert "form_component" in result.missing

    def test_three_of_four_controlled_passes(self):
        code = COMBINED_CODE.replace("ControlledTextarea", "PlainTextarea")
        assert "controlled_components" not in validate_ast_structure(code, SPEC).missing

    def test_two_of_four_controlled_fails(self):
        code = COMBINED_CODE.replace("ControlledTextarea", "PT").replace("ControlledCheckbox", "PC")
    ...
    def test_missing_v_if_reduces_score(self):
        assert "conditional_rendering" in validate_ast_structure(
            COMBINED_CODE.replace("v-if", "x-rm"), SPEC
        ).missing

    def test_all_required_fields_pass(self):
        assert "required_fields" not in validate_ast_structure(COMBINED_CODE, SPEC).missing

    def test_two_of_three_conditional_passes(self):
        code = COMBINED_CODE.replace("otherInfo", "REMOVED")
