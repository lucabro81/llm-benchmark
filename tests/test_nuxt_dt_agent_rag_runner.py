"""Tests for src/agent/nuxt_dt_agent_rag/test_runner.py.

Key differences from nuxt-dt-agent-twofiles:
...
from src.agent.nuxt_dt_agent_rag.test_runner import AgentBenchmarkResult, AgentTest
...
def _make_fixture(tmp_path: Path, spec=None) -> Path:
    ...
    fixture_path = tmp_path / "nuxt-dt-agent-rag"
...
def _make_agent_result(**kwargs):
    defaults = dict(
        ...
        first_compile_success_step=2, compile_error_recovery_count=0,
        rag_queries_count=1, read_file_count=0, list_files_count=0,
        run_crashed=False,
    )
...
class TestAgentTestTools:
    ...
    @patch("src.agent.nuxt_dt_agent_rag.test_runner.validator")
    @patch("src.agent.nuxt_dt_agent_rag.test_runner.run_agent")
...
class TestWriteFileDecoupled:
    ...
    def test_write_file_returns_file_written_only(self, tmp_path):
        ...
        from src.agent.nuxt_dt_agent_rag.test_runner import _make_tools
