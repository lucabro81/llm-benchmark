"""Tests for src/agent/nuxt_form_agent_rag/validator.py.
...
from src.agent.nuxt_form_agent_rag.validator import (
    ASTResult,
    CompilationResult,
    NamingResult,
...
class TestValidateAstStructure:
    ...
    def test_missing_script_lang_reduces_score(self):
        assert "script_lang" in validate_ast_structure(
            COMPLETE_CODE.replace('lang="ts"', 'lang="js"'), SPEC
        ).missing

    def test_missing_form_component_reduces_score(self):
        assert "form_component" in validate_ast_structure(
            COMPLETE_CODE.replace("<Form ", "<FormWrapper "), SPEC
        ).missing
