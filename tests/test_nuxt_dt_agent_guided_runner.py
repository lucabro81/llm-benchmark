"""Tests for src/agent/nuxt_dt_agent_guided/test_runner.py.

Key differences from nuxt-form guided runner:
...
- max_steps: 10
- _make_fixture also creates a columns.ts stub (but guided only writes OrdersDataTable.vue)
"""
...
from src.agent.nuxt_dt_agent_guided.test_runner import AgentBenchmarkResult, AgentTest
...
DEFAULT_SPEC = {
    ...
    "allowed_write_paths": ["apps/web/src/orders/OrdersDataTable.vue"],
...
def _make_fixture(tmp_path: Path, spec=None) -> Path:
    ...
    fixture_path = tmp_path / "nuxt-dt-agent-guided"
...
class TestAgentTestTools:
    ...
    @patch("src.agent.nuxt_dt_agent_guided.test_runner.validator")
    @patch("src.agent.nuxt_dt_agent_guided.test_runner.run_agent")
...
class TestAgentTestRun:
    ...
    @patch("src.agent.nuxt_dt_agent_guided.test_runner.validator")
    @patch("src.agent.nuxt_dt_agent_guided.test_runner.run_agent")
    def test_stub_restored_after_run(self, mock_run_agent, mock_validator, tmp_path):
        fixture_path = _make_fixture(tmp_path)
...
class TestWriteFileDecoupled:
    ...
    def test_write_file_returns_file_written_only(self, tmp_path):
        ...
        from src.agent.nuxt_dt_agent_guided.test_runner import _make_tools
