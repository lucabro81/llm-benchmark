"""Creation test orchestrator for the nuxt-dt-oneshot fixture.
...
- target_project resolved via target_project_path in validation_spec.json (relative to fixture dir)
  so it can point to the shared monorepo used by all nuxt-dt fixtures
- Compilation uses compilation_cwd + compilation_command (npm run check-types from apps/web)
- columns.ts and types.ts state saved/restored defensively (shared target_project may be in use)
"""
...
from src.creation.nuxt_dt_oneshot import validator
from src.creation.nuxt_dt_oneshot.validator import ASTResult, NamingResult
...
class BenchmarkResult:
    """Complete test execution result for the nuxt-dt-oneshot fixture."""
...
class CreationTest:
    """Single-shot creation test for the nuxt-dt-oneshot fixture.

    The model receives the complete DataTable component API docs inline in the prompt
    and must output a single OrdersDataTable.vue with types defined inline.
    """
    ...
    def __init__(self, model: str, fixture_path: Path, prompt_version: str | None = None):
        ...
        # Save types.ts (provided, not a write target, but restore defensively)
