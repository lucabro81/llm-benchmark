...
- query_rag tool added (BM25 over rag_docs/)
- validate_ast_structure receives combined code from both target files
"""
...
from src.agent.veevalidate_zod_form_nuxt_rag import validator
from src.agent.veevalidate_zod_form_nuxt_rag.rag import QueryRagTool
from src.agent.veevalidate_zod_form_nuxt_rag.validator import ASTResult, NamingResult
...
class AgentBenchmarkResult:
    """Complete agent test execution result for the nuxt-rag fixture."""
...
def _make_tools(
    ...
    def read_file(path: str) -> str:
        ...
        Args:
            path: Relative path from the project root (e.g. 'apps/web/src/registration/components/RegistrationForm.vue').
...
class AgentTest:
    ...
    def __init__(self, model: str, fixture_path: Path):
        ...
        # Second stub file (types/index.ts)
    ...
    def run(self, run_number: int = 1) -> AgentBenchmarkResult:
        ...
        try:
            ...
            # 4. Read final state from both files (combined for validation)
            output_code = ""
            types_code = ""
            try:
