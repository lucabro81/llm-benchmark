...
Scoring breakdown (validate_ast_structure, 0-10):
  ...
  +2  zod_schema            z.object( present (may be in types file)
...
def validate_ast_structure(code: str, expected_patterns: dict) -> ASTResult:
    """Validate component + types patterns using regex on combined source text.

    Args:
        code: Combined source (RegistrationForm.vue + types/index.ts content).
    ...
    # --- form_component (+1) ---
    # Require the tag name to end with a non-word char to avoid matching prefixes
    # e.g. "<Form " matches, "<FormWrapper " does not.
...
def validate_naming(code: str, conventions: dict) -> NamingResult:
    ...
    Args:
        code: Combined source code (component + types).
