"""Agent test orchestrator for the nuxt-dt-agent-rag fixture.

Differences from nuxt-dt-agent-guided:
...
- Tools: write_file + run_compilation + query_rag (no read_file, no list_files)
  The model must query RAG to get DataTable API docs, then compose the component.
- Target files: OrdersDataTable.vue + columns.ts
- max_steps: 20
"""
...
from src.agent.nuxt_dt_agent_rag import validator
from src.agent.nuxt_dt_agent_rag.rag import QueryRagTool
from src.agent.nuxt_dt_agent_rag.validator import ASTResult, NamingResult
...
class AgentBenchmarkResult:
    """Complete agent test execution result for the nuxt-dt-agent-rag fixture."""
...
class AgentTest:
    """Agent benchmark test for the nuxt-dt-agent-rag fixture.

    The model has no injected context and must use query_rag to retrieve DataTable API docs,
    then write + compile to implement the component. Tests: retrieval + composition + error recovery.
    """
