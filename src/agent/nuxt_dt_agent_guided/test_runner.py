"""Agent test orchestrator for the nuxt-dt-agent-guided fixture.
...
- Tools: ONLY write_file + run_compilation (no read_file, no list_files, no query_rag)
  The model has full API docs injected in the prompt — no exploration needed.
- Target file: apps/web/src/orders/OrdersDataTable.vue
- columns.ts and types.ts stubs saved/restored defensively
- max_steps: 10 (generous for write→compile→fix cycle, but tight enough to penalise waste)
"""
...
from src.agent.nuxt_dt_agent_guided import validator
from src.agent.nuxt_dt_agent_guided.validator import ASTResult, NamingResult
...
class AgentBenchmarkResult:
    """Complete agent test execution result for the nuxt-dt-agent-guided fixture."""
...
def _make_tools(
    ...
    """Build tools for the guided fixture: ONLY write_file + run_compilation.

    No read_file, no list_files, no query_rag — the model has all context in the prompt.
    """
...
class AgentTest:
    """Agent benchmark test for the nuxt-dt-agent-guided fixture.

    The model receives complete API docs in the prompt and must write + fix TS errors.
    No exploration tools available — tests composition + error recovery in isolation.
    """
    ...
    def __init__(self, model: str, fixture_path: Path, prompt_version: str | None = None):
        ...
        self.max_steps = self.validation_spec.get("max_steps", 10)
    ...
    def run(self, run_number: int = 1, prompt_log_path: "Path | None" = None) -> AgentBenchmarkResult:
        ...
        try:
            ...
            # 2. Build tools (write + compile only)
            tools = _make_tools(
            ...
            # 3b. Early exit if agent crashed before making any tool calls.
            # Skip validation entirely — stub file is unchanged and scores would be misleading.
            ...
            # 4. Read final state (guided: single file only)
            output_code = ""
            try:
