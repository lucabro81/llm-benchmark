"""Agent test orchestrator for the nuxt-dt-agent-twofiles fixture.

Differences from nuxt-dt-agent-guided (Test F):
- The model writes TWO files in order:
    1. apps/web/src/orders/columns.ts  (column definitions with renderers)
    2. apps/web/src/orders/OrdersDataTable.vue  (imports from columns)
- output_code in result contains both files combined (Vue + separator + columns)
...
from src.agent.nuxt_dt_agent_twofiles import validator
from src.agent.nuxt_dt_agent_twofiles.validator import ASTResult, NamingResult
...
class AgentBenchmarkResult:
    """Complete agent test execution result for the nuxt-dt-agent-twofiles fixture."""
...
def _make_tools(
    ...
    def write_file(path: str, content: str) -> str:
        ...
        Args:
            path: Relative path to write (e.g. 'apps/web/src/orders/columns.ts').
...
class AgentTest:
    """Agent benchmark test for the nuxt-dt-agent-twofiles fixture.

    The model receives complete API docs in the prompt and must write two files:
    columns.ts (column definitions + renderers) then OrdersDataTable.vue (imports from columns).
