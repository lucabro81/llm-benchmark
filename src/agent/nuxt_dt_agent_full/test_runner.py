"""Agent test orchestrator for the nuxt-dt-agent-full fixture (Test J).

Differences from the base agent runner:
- Three stub files restored (OrdersDataTable.vue + columns.ts + types.ts)
- Custom tools: compilation runs from apps/web/ with 'npm run check-types'
- Full tool set: read_file + write_file + list_files + run_compilation + query_rag
- validate_ast_structure receives combined code from all target files
"""
...
from src.agent.nuxt_dt_agent_full import validator
from src.agent.nuxt_dt_agent_full.rag import QueryRagTool
from src.agent.nuxt_dt_agent_full.validator import ASTResult, NamingResult
...
class AgentBenchmarkResult:
    """Complete agent test execution result for the nuxt-dt-agent-full fixture."""
...
def _make_tools(
    ...
    """Build tools for this fixture: read/write/list/compile + query_rag.

    write_file and run_compilation use the fixture-specific cwd and command.
    """
    ...
    def read_file(path: str) -> str:
        ...
        Args:
            path: Relative path from the project root (e.g. 'apps/web/src/orders/types.ts').
    ...
    def write_file(path: str, content: str) -> str:
        ...
        Args:
            path: Relative path to write (e.g. 'apps/web/src/orders/OrdersDataTable.vue').
    ...
    return [read_file, write_file, list_files, run_compilation, rag_tool]


class AgentTest:
    """Agent benchmark test orchestrator for the nuxt-dt-agent-full fixture."""
    ...
    def __init__(self, model: str, fixture_path: Path, prompt_version: str | None = None):
        ...
        columns_rel = "apps/web/src/orders/columns.ts"
        self._columns_file = self.target_project / columns_rel
        self.original_columns = self._columns_file.read_text() if self._columns_file.exists() else ""

        types_rel = "apps/web/src/orders/types.ts"
        ...
        self.max_steps = self.validation_spec.get("max_steps", 30)
    ...
    def run(self, run_number: int = 1, prompt_log_path: "Optional[Path]" = None) -> AgentBenchmarkResult:
        ...
        try:
            ...
            if self._columns_file.exists() or self.original_columns:
                self._columns_file.write_text(self.original_columns)
            ...
            # 4. Read final state from all three files (combined for validation)
            output_code = ""
            columns_code = ""
            types_code = ""
            try:
            ...
            try:
                if self._columns_file.exists():
                    columns_code = self._columns_file.read_text()
            except Exception:
                pass
            try:
            ...
            # 5b. Pattern check (on combined code)
            ...
            # 5c. Naming check (on combined code)
